...
export const UploadFiles = () => {
  ...
  const handleConfirmClick = () => {
    ...
    const allSpecsSelected = Object.values(specifications).every(value => value.trim() !== '');
    if (!allSpecsSelected) {
  ...
  const specs = [
    { label: "Paper size:", options: ["A4", "Short", "Long"], stateKey: "paperSize" },
    { label: "Printing options:", options: ["Full color", "Black&White"], stateKey: "printOption" },
    { label: "Turnaround Time:", options: ["Standard", "Rush"], stateKey: "turnaroundTime" },
    { label: "Payment method:", options: ["Cash on Delivery", "Gcash"], stateKey: "paymentMethod" },
  ];
          ...
          <div className="uf-step-circles">
            {steps.map((step, index) => (
              <React.Fragment key={step.number}>
                {index > 0 && (
                  <div className={`uf-line ${steps[index - 1].active ? 'active' : ''}`}></div>
                )}
                <div className={`uf-step-circle ${step.active ? "active" : ""}`}>
            ...
            {selectedFile ? (
              <div className="uf-file-preview">
                <h3 className="uf-preview-title">
                  Uploading Files
                  <span className="uf-close-preview" onClick={() => setSelectedFile(null)}>×</span>
                </h3>
              ...
              <>
                <img src={uploadImage} alt="upload" className="uf-upload-icon" />
                ...
                {specs.map((spec, index) => (
                  <div key={index} className="uf-spec-item">
                    ...
                    <select
                      ...
                      onChange={(e) => handleSpecChange(spec.stateKey, e.target.value)}
                    >
                    ...
                    {specifications[spec.stateKey] && (
                      <div className="uf-selected-value">
                      </div>
                    )}
                  </div>
                ))}
              ...
              <div className="uf-payment-section">
                <div className="uf-payment-info">
                  ...
                  <div className="uf-payment-amount">
                    <img src={uploadPeso} alt="Currency" className="uf-payment-icon" />
                    <span className="uf-payment-value">00.00</span>
                  </div>
                </div>
                <div className="uf-confirm-btn-wrapper">
                  <button className="uf-btn-primary" onClick={handleConfirmClick}>Confirm</button>
                </div>
              </div>
            </div>
          </div>
